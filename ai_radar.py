...
if "ai_model" not in st.session_state:
    st.session_state.ai_model = "OpenAI" # NEW: Initialize AI model choice
...
# Data functions (unchanged from previous versions)
...
# Main content
tabs = st.tabs(["Dashboard", "Market Movers", "My Watchlist", "ETF Tracker", "Auto Plays", "Settings"])
...
with tabs[0]:
    ...
    if st.button("Generate Market Analysis", use_container_width=True, type="primary"):
        with st.spinner(f"Generating market analysis with {st.session_state.ai_model}..."):
            ...
            movers = get_top_gainers_losers_volume_multithreaded()
            analysis_text = ai_market_analysis(news_items, movers, st.session_state.ai_model)
            st.markdown(analysis_text)
    
    # Auto-Generated Plays
    st.markdown("---")
    st.markdown(f"**Auto-Generated Trading Plays** powered by **{st.session_state.ai_model}**")
    if st.button("Generate Auto Plays", use_container_width=True, type="primary"):
...
# TAB 5: Auto Plays
with tabs[4]:
    st.subheader(f"✨ Auto-Generated Plays")
...
with tabs[5]:
    ...
    # API Key Input (for local testing)
