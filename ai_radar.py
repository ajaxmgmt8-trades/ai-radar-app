...
# Expanded market scanning list for broader market movers
MARKET_SCAN_TICKERS = [
    # Major Indices
    "SPY", "QQQ", "IWM", "DIA", "VTI", "SPX", "NDX",
    # Mega Cap Tech
    "AAPL", "MSFT", "GOOGL", "GOOG", "AMZN", "NVDA", "META", "TSLA", "NFLX", "ADBE",
    # Large Cap Growth
    "CRM", "SNOW", "PLTR", "ZM", "DOCU", "RBLX", "ROKU", "TWLO", "DDOG", "CRWD",
    # Large Cap Value  
    "BRK-B", "JPM", "BAC", "WFC", "JNJ", "PG", "KO", "PFE", "XOM", "CVX",
    # Tech/Semiconductor
    "AMD", "INTC", "TSM", "AVGO", "MU", "QCOM", "TXN", "AMAT", "LRCX", "KLAC",
    # Growth Stocks
    "SHOP", "SQ", "PYPL", "ADSK", "WDAY", "NOW", "OKTA", "MDB", "TEAM", "SPOT",
    # Healthcare/Biotech
    "MRNA", "PFE", "JNJ", "UNH", "CVS", "ABBV", "TMO", "DHR", "ISRG", "VRTX",
    # Energy
    "XLE", "USO", "XOM", "CVX", "COP", "EOG", "SLB", "OXY", "MPC", "VLO",
    # Finance
    "XLF", "JPM", "BAC", "WFC", "GS", "MS", "C", "USB", "TFC", "PNC",
    # Consumer
    "AMZN", "HD", "WMT", "TGT", "LOW", "COST", "NKE", "SBUX", "MCD", "DIS",
    # Communication
    "GOOGL", "META", "NFLX", "DIS", "CMCSA", "VZ", "T", "TMUS", "CHTR", "PARA",
    # Industrial
    "CAT", "BA", "GE", "HON", "UPS", "FDX", "LMT", "RTX", "MMM", "DE",
    # Real Estate
    "IYR", "VNQ", "AMT", "PLD", "CCI", "EQIX", "PSA", "EXR", "O", "REYN",
    # Materials
    "XLB", "LIN", "APD", "ECL", "SHW", "NEM", "FCX", "AA", "X", "CLF",
    # Utilities
    "XLU", "NEE", "DUK", "SO", "AEP", "EXC", "SRE", "PEG", "ED", "ES",
    # Crypto Related
    "MSTR", "COIN", "RIOT", "MARA", "HOOD", "SQ", "PYPL", "BITF", "HUT", "WULF",
    # Meme/Retail Favorites
    "GME", "AMC", "BBBY", "SNDL", "NOK", "BB", "WISH", "CLOV", "SPCE", "PLTR",
    # Chinese ADRs
    "BABA", "JD", "PDD", "BIDU", "NIO", "XPEV", "LI", "TME", "BILI", "IQ",
    # Other Popular
    "UBER", "LYFT", "DASH", "ABNB", "ZG", "ZILLOW", "PELOTON", "ZOOM", "DOCN", "DKNG"
]
...
def generate_unique_key(prefix: str, identifier: str = "", index: int = None, timestamp: bool = False) -> str:
    ...
    if index is not None:
        base_key += f"_{index}"
    if timestamp:
        base_key += f"_{int(time.time() * 1000) % 100000}"
    return base_key
...
@st.cache_data(ttl=1800)  # Cache for 30 minutes (reduced from 1 hour)
...
@st.cache_data(ttl=30)  # Cache for 30 seconds (reduced from 60)
...
@st.cache_data(ttl=300)  # Cache for 5 minutes (reduced from 10)
...
def ai_auto_generate_plays_enhanced(tz: str):
    ...
    try:
        ...
        scan_tickers = list(set(current_watchlist + MARKET_SCAN_TICKERS[:60]))  # Use broader market scan
...
with col2:
    st.session_state.refresh_interval = st.selectbox("Interval", [10, 15, 30], index=2)  # Faster intervals
...
with tabs[0]:
    ...
    if search_quotes and search_ticker:
        with st.spinner(f"Getting quote for {search_ticker}..."):
            ...
            if not quote["error"]:
                ...
                if col4.button(f"📊 Enhanced Analysis", key=generate_unique_key("quotes_enhanced", search_ticker)):
                ...
                if col4.button(f"Add {search_ticker} to Watchlist", key=generate_unique_key("quotes_add_searched_ticker", search_ticker)):
        ...
        for i, ticker in enumerate(tickers):
            ...
            with st.container():
                ...
                if abs(quote['change_percent']) >= 2.0:
                    if col4.button(f"🎯 AI Analysis", key=generate_unique_key("quotes_ai", ticker, i)):
    ...
    # Top Market Movers - Updated to use broader market scan
    st.markdown("### 🌟 Top Market Movers")
    st.caption("Stocks with significant intraday movement from entire market scan")
    movers = []
    
    # Use broader MARKET_SCAN_TICKERS instead of just CORE_TICKERS
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        future_to_ticker = {executor.submit(get_live_quote, ticker, tz_label): ticker for ticker in MARKET_SCAN_TICKERS[:100]}  # Scan top 100
        for future in concurrent.futures.as_completed(future_to_ticker):
            ticker = future_to_ticker[future]
            try:
                quote = future.result()
                if not quote["error"] and quote["volume"] > 500000:  # Minimum volume filter
                    mover_data = {
                        "ticker": ticker,
    ...
    top_movers = movers[:15]  # Show top 15 movers
    ...
    for i, mover in enumerate(top_movers):
        with st.container():
            ...
            if col4.button(f"Add {mover['ticker']} to Watchlist", key=generate_unique_key("quotes_mover_add", mover['ticker'], i)):
                current_list = st.session_state.watchlists[st.session_state.active_watchlist]
...
with tabs[1]:
    ...
    for i, ticker in enumerate(CORE_TICKERS):
        with cols[i % 6]:
            if st.button(f"+ {ticker}", key=generate_unique_key("watchlist_add", ticker, i)):
    ...
    if current_tickers:
        for i in range(0, len(current_tickers), 5):
            ...
            for j, ticker in enumerate(current_tickers[i:i+5]):
                with cols[j]:
                    st.write(f"**{ticker}**")
                    if st.button(f"Remove", key=generate_unique_key("watchlist_remove", ticker, i * 5 + j)):
...
with tabs[2]:
    ...
    if search_catalyst and search_catalyst_ticker:
        with st.spinner(f"Searching all news sources for {search_catalyst_ticker} catalysts..."):
            ...
            if not quote["error"]:
                ...
                if st.button(f"Add {search_catalyst_ticker} to Watchlist", key=generate_unique_key("catalyst_add_searched_ticker", search_catalyst_ticker)):
                    current_list = st.session_state.watchlists[st.session_state.active_watchlist]
    ...
    if st.button("🔍 Scan Market Catalysts", type="primary"):
        with st.spinner("Scanning all news sources for market catalysts..."):
            ...
            # Get significant movers for correlation - use broader market scan
            movers = []
            with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
                future_to_ticker = {executor.submit(get_live_quote, ticker, tz_label): ticker for ticker in MARKET_SCAN_TICKERS[:50]}
                for future in concurrent.futures.as_completed(future_to_ticker):
                    ticker = future_to_ticker[future]
                    try:
                        quote = future.result()
            ...
            if movers:
                ...
                for i, mover in enumerate(movers[:10]):
                    ...
                    with col3:
                        if st.button(f"📰 News", key=generate_unique_key("catalyst_news", mover['ticker'], i)):
                        ...
                        if st.button(f"Add", key=generate_unique_key("catalyst_add_mover", mover['ticker'], i)):
                            current_list = st.session_state.watchlists[st.session_state.active_watchlist]
                            if mover['ticker'] not in current_list:
                                current_list.append(mover['ticker'])
                                st.session_state.watchlists[st.session_state.active_watchlist] = current_list
...
with tabs[3]:
    ...
    if search_analysis and search_analysis_ticker:
        with st.spinner(f"AI analyzing {search_analysis_ticker}..."):
            ...
            if not quote["error"]:
                ...
                if col4.button(f"Add {search_analysis_ticker} to WL", key=generate_unique_key("analysis_add_searched_ticker", search_analysis_ticker)):
                    current_list = st.session_state.watchlists[st.session_state.active_watchlist]
    ...
    if st.button("🤖 Generate Market Analysis", type="primary"):
        with st.spinner("AI analyzing market conditions..."):
            ...
            # Use broader market scan for movers
            movers = []
            with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
                future_to_ticker = {executor.submit(get_live_quote, ticker, tz_label): ticker for ticker in MARKET_SCAN_TICKERS[:30]}
                for future in concurrent.futures.as_completed(future_to_ticker):
                    ticker = future_to_ticker[future]
                    try:
                        quote = future.result()
                ...
                if auto_plays:
                    ...
                    for i, play in enumerate(auto_plays):
                        with st.expander(f"🎯 {play['ticker']} - ${play['current_price']:.2f} ({play['change_percent']:+.2f}%) | {play.get('data_source', 'Yahoo Finance')}"):
                            ...
                            if st.button(f"Add {play['ticker']} to Watchlist", key=generate_unique_key("playbook_auto", play['ticker'], i)):
                                current_list = st.session_state.watchlists[st.session_state.active_watchlist]
    ...
    if search_playbook and search_playbook_ticker:
        ...
        if not quote["error"]:
            with st.spinner(f"AI generating playbook for {search_playbook_ticker}..."):
                ...
                col3.metric("Volume", f"{quote['volume']:,}")
                if col4.button(f"Add {search_playbook_ticker} to WL", key=generate_unique_key("playbook_add_searched_ticker", search_playbook_ticker)):
                    current_list = st.session_state.watchlists[st.session_state.active_watchlist]
...
with tabs[5]:
    ...
    for i, ticker in enumerate(st.session_state.etf_list):
        quote = get_live_quote(ticker, tz_label)
        if quote["error"]:
            st.error(f"{ticker}: {quote['error']}")
            continue
        ...
        with st.container():
            ...
            if col4.button(f"Add {ticker} to Watchlist", key=generate_unique_key("sector_etf_add", ticker, i)):
                current_list = st.session_state.watchlists[st.session_state.active_watchlist]
    ...
    if st.button("📊 Get Enhanced Earnings Plays", type="primary"):
        with st.spinner("AI analyzing earnings reports with enhanced data..."):
            ...
            if not earnings_today:
                ...
                for i, report in enumerate(earnings_today):
...
with tabs[10]:
    ...
    if not grok_enhanced:
        ...
        if analyze_social and social_ticker:
            with st.spinner(f"Grok analyzing Twitter/X sentiment for {social_ticker}..."):
                try:
                    ...
                    # Add to watchlist option
                    if st.button(f"Add {social_ticker} to Watchlist", key=generate_unique_key("twitter_add_searched_ticker", social_ticker)):
                        current_list = st.session_state.watchlists[st.session_state.active_watchlist]
        ...
        for i, ticker in enumerate(popular_for_social):
            with cols[i % 4]:
                if st.button(f"📊 {ticker}", key=generate_unique_key("twitter_quick_social", ticker, i)):
