...
class UnusualWhalesClient:
    """FIXED Unusual Whales API client using documented endpoints"""
    
    def __init__(self, api_key: str):
        self.api_key = api_key
        self.base_url = "https://api.unusualwhales.com"  # FIXED: Correct API endpoint
        self.session = requests.Session()
        self.session.headers.update({
            "Authorization": f"Bearer {api_key}",  # FIXED: Proper Bearer token format
    ...
    def _make_request(self, endpoint: str, params: Dict = None) -> Dict:
        """FIXED: Make authenticated request to UW API"""
        try:
            url = f"{self.base_url}{endpoint}"
            
            # FIXED: Add API key to params as well for redundancy
            if params is None:
                params = {}
            params['apikey'] = self.api_key
            
            response = self.session.get(url, params=params, timeout=15)  # FIXED: Increased timeout
            ...
            if response.status_code == 200:
                try:
                    ...
                    return {"error": "Invalid JSON response", "status_code": 200}
    ...
    def get_quote(self, symbol: str) -> Dict:
        """FIXED: Get stock quote using UW stock API"""
        try:
            # FIXED: Use documented stock endpoint
            result = self._make_request(f"/api/stock/{symbol}/stock-state")
            
            if result.get("error"):
                # FIXED: Fallback to basic stock endpoint
            ...
            # FIXED: Parse UW stock data structure with multiple field names
            if isinstance(data, dict):
                # FIXED: Extract price data with flexible field mapping
    ...
    def _extract_price(self, data: Dict) -> float:
        """FIXED: Extract current price from various possible field names"""
        price_fields = [
    ...
    def _extract_prev_close(self, data: Dict) -> float:
        """FIXED: Extract previous close price"""
        prev_fields = [
            "prev_close_price", "previousClose", "prev_close", "prevClose", "yesterday_close",
    ...
    def _extract_volume(self, data: Dict) -> int:
        """FIXED: Extract volume from various possible field names"""
        volume_fields = [
    ...
    def _extract_premarket_change(self, data: Dict) -> float:
        """FIXED: Extract premarket change if available"""
        pm_fields = [
    ...
    def _extract_postmarket_change(self, data: Dict) -> float:
        """FIXED: Extract after hours change if available"""
        ah_fields = [
    ...
    def get_options_volume(self, symbol: str) -> Dict:
        """FIXED: Get options volume data using documented endpoint"""
    ...
    def get_flow_alerts(self, symbol: str) -> Dict:
        """FIXED: Get options flow alerts using documented endpoint"""
    ...
    def get_greek_exposure(self, symbol: str) -> Dict:
        """FIXED: Get Greek exposure data using documented endpoint"""
    ...
    def get_market_tide(self) -> Dict:
        """FIXED: Get overall market sentiment using documented endpoint"""
    ...
    def get_institutional_activity(self, symbol: str) -> Dict:
        """FIXED: Get institutional activity using documented endpoints"""
        # FIXED: Try congress endpoint first
        result = self._make_request(f"/api/congress/recent-trades", {"ticker": symbol})
        ...
        # FIXED: Try insider endpoint as fallback
        result = self._make_request(f"/api/institution/{symbol}/activity")
    ...
    def test_connection(self) -> Dict:
        """FIXED: Test API connection and permissions"""
    ...
    def analyze_trading_setup(self, prompt: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model="grok-beta",  # FIXED: Use correct Grok model name
                messages=[{
                    "role": "system", 
...
def get_live_quote(ticker: str, tz: str = "ET") -> Dict:
    """
    FIXED: Get live stock quote using Unusual Whales FIRST, then fallbacks
    """
...
def ai_market_analysis(news_items: List[Dict], movers: List[Dict]) -> str:
    """Standard market analysis function"""
...
.upper().strip()
    ...
    if current_tickers:
        ...
        st.info("No stocks in watchlist. Add some using the search above or popular tickers.")

# Continue with remaining tabs...
with tabs[2]:
    st.subheader("🔥 Catalyst Scanner - Coming Soon")
    st.info("Enhanced catalyst scanner with Unusual Whales integration will be available in the next update.")

with tabs[3]:
    st.subheader("📈 Market Analysis - Coming Soon")
    st.info("AI-powered market analysis will be available in the next update.")

with tabs[4]:
    st.subheader("🤖 AI Playbooks - Coming Soon")
    st.info("High-accuracy AI trading playbooks will be available in the next update.")

with tabs[5]:
    st.subheader("🌐 Sector/ETF Tracking - Coming Soon")
...
with tabs[8]:
    st.subheader("📰 Important News - Coming Soon")
    st.info("Important news and economic calendar will be available in the next update.")

with tabs[9]:
    st.subheader("🐦 Twitter/X Sentiment - Coming Soon")
    st.info("Social media sentiment analysis will be available in the next update.")
...
st.markdown(
    ...
    f"🔥 AI Radar Pro v2.0 FIXED | Data: {footer_text} | {ai_footer} | 🎯 Targeting 80-100% Accuracy"
    "</div>",
